...
module.exports = function(root, opts) {
  ...
  if(!opts.typeThreeDuplication) return {
  ...
  root.walkRules((rule) => {
    ...
    rule.nodes.forEach((declaration) => {
      ...
      if(declaration.prop && declaration.value) {
        ...
        if (shorthandProperties.includes(prop)) {
          ...
          shorthandHashes.push({hash: hash, selector: selector, line: rule.source.start.line, file: rule.source.input.file})
    ...
    properties.forEach((propArr) => {
      if(propArr.length) {
        ...
        possibleLonghandHashes.push({hash: hash, selector: selector, line: rule.source.start.line, file: rule.source.input.file})
      } 
    })
